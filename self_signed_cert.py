...
def create_self_signed_cert(cert_dir='.'):
    ...
    if not exists(C_F) or not exists(K_F):
        ...
        open(C_F, 'wt').write(crypto.dump_certificate(
            crypto.FILETYPE_PEM, cert).decode('utf8'))
        open(K_F, 'wt').write(crypto.dump_privatekey(
            crypto.FILETYPE_PEM, k).decode('utf8'))
